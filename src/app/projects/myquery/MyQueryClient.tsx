        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="relative">
            <h3 className="text-4xl font-black tracking-tighter text-white flex flex-col items-center leading-none">
              <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-sinai-glow-orange/50 uppercase">MYQUERY</span>
              <span className="text-[10px] font-mono tracking-[1.2em] text-sinai-glow-orange/60 ml-[1.2em] mt-2 font-bold uppercase">Analytics</span>
            </h3>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="relative w-28 h-28 flex items-center justify-center cursor-pointer">
        ...
        <div 
          style={{ width: radius, transform: `rotate(${angle + Math.PI}rad)`, transformOrigin: 'left center' }}
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center bg-white/[0.02] backdrop-blur-3xl border border-white/10 p-12 rounded-[4rem] relative overflow-hidden group">
      ...
      <div className="space-y-8 relative z-10">
        <div className="flex items-center gap-4">
          <div className="px-4 py-1.5 rounded-full bg-sinai-glow-orange/10 border border-sinai-glow-orange/30 text-sinai-glow-orange text-[9px] font-mono font-bold tracking-widest uppercase">MyQuery_Core // {cap.id}</div>
...
const METRICS = [
  { label: "Insights Velocity", value: "Instant", detail: "Real-time parsing" },
  { label: "Data Integrity", value: "99.2%", detail: "Accuracy score" },
  { label: "Integrations", value: "22+", detail: "Native connectors" },
  { label: "Setup Time", value: "5 min", detail: "Fast deployment" }
];
...
const PIPELINE_STEPS = [
  {
    step: "01",
    title: "Unified Sync",
    desc: "Connect MyQuery to your disparate data sources. Our universal bridge aggregates metadata to create a unified semantic map of your entire enterprise data stack.",
  ...
  },
  {
    ...
    desc: "Every query passes through our security and governance engine. We verify permissions and apply data masking in real-time to ensure full compliance with enterprise standards.",
    ...
    <div ref={containerRef} className="relative bg-[#06080a] text-white min-h-screen selection:bg-sinai-glow-orange selection:text-black font-sans">
      ...
      <section className="relative min-h-screen flex items-center pt-32 overflow-hidden bg-black">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, rotate: 5 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: 1, ease: "easeOut", delay: 0.2 }} className="relative">
              <div className="relative aspect-square max-w-2xl mx-auto rounded-[4rem] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(242,162,75,0.15)] group bg-zinc-950">
                <Image src="/images/myquery-premium.png" alt="MyQuery Interface" fill className="object-cover opacity-80 group-hover:scale-110 transition-transform duration-[5000ms]" priority />
                ...
                <div className="absolute top-12 left-12 p-6 rounded-2xl bg-black/60 backdrop-blur-xl border border-white/10 max-w-[200px] space-y-4">
                  <div className="text-[8px] font-mono text-sinai-glow-orange tracking-widest font-black uppercase">Insights_Engine_Live</div>
                ...
                <div className="absolute bottom-12 right-12 text-right">
                  <div className="text-[10px] font-mono text-zinc-500 tracking-[0.2em] mb-2 uppercase">Neural_Core</div>
                  <div className="w-48 h-12 relative">
                    <Image src="/images/myquery-logo.png" alt="Logo" fill className="object-contain object-right brightness-0 invert opacity-40" />
                  </div>
                </div>
              </div>
            </motion.div>
              ...
              <div className="space-y-16">
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sinai-glow-orange font-mono text-[10px] tracking-[0.5em] font-bold uppercase">The_Data_Gap</span>
                  </div>
                  <h2 className="text-6xl md:text-7xl font-black tracking-tighter leading-none uppercase">Information is <br /><span className="text-zinc-800">Siloed.</span></h2>
                </div>
                <div className="space-y-10">
                  <div className="p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 backdrop-blur-xl relative">
                    <p className="text-zinc-400 leading-relaxed font-light text-xl">Modern enterprises are drowning in data but starving for insights. Data teams are overwhelmed with repetitive requests, while business stakeholders wait weeks for reports that should take seconds.</p>
                  </div>
                  <div className="flex flex-col gap-6">
                    {["Fragmented Data Sources", "Infinite Report Backlogs", "Manual SQL Engineering", "Stale Dashboard Sync"].map((item, i) => (
                      <div key={i} className="flex items-center gap-6 group">
                        <div className="w-12 h-12 rounded-xl bg-red-500/5 border border-red-500/10 flex items-center justify-center text-red-500/40 group-hover:text-red-500 transition-colors"><Zap className="w-5 h-5" /></div>
            ...
            <SectionReveal>
              <div className="space-y-16">
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-px bg-sinai-glow-orange/30" />
                    <span className="text-sinai-glow-orange font-mono text-[10px] tracking-[0.5em] font-bold uppercase">The_Platform</span>
                  </div>
                  <h2 className="text-6xl md:text-7xl font-black tracking-tighter leading-none uppercase">MyQuery is the <br /><span className="text-white">Neural Hub.</span></h2>
                </div>
                <div className="space-y-10">
                  <div className="p-10 rounded-[3rem] bg-sinai-glow-orange/5 border border-sinai-glow-orange/20 backdrop-blur-xl relative overflow-hidden group">
                    ...
                    <p className="text-zinc-300 leading-relaxed font-light text-xl relative z-10">MyQuery provides a single conversational interface for your entire data stack. Connect to Snowflake, BigQuery, or Redshift and start getting answers in seconds, fully governed and secured.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[{ title: "Cross-Cloud Analytics", desc: "Unified semantic map" }, { title: "Automated Governance", desc: "Role-based security" }, { title: "Native Connectors", desc: "22+ Data sources" }, { title: "Executive UX", desc: "Premium dashboards" }].map((item, i) => (
          ...
          <SectionReveal>
            <div className="text-center mb-32">
              <div className="inline-flex items-center gap-4 mb-8">
                ...
                <span className="text-sinai-glow-orange font-mono text-[10px] tracking-[0.5em] font-bold uppercase">Core_Capabilities // 03</span>
                <div className="w-8 h-px bg-sinai-glow-orange/50" />
              </div>
              <h2 className="text-6xl md:text-8xl font-black mb-10 tracking-tight leading-none uppercase">The Intelligence <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-sinai-glow-orange to-white/40">Powering MyQuery</span></h2>
            </div>
          </SectionReveal>
          <div className="relative max-w-7xl mx-auto h-[700px] flex items-center justify-center">
            <div className="absolute inset-0 z-30 pointer-events-none">
              {FEATURES.map((feature, i) => (
                <OrbitalNode key={feature.id} cap={feature} index={i} total={FEATURES.length} active={activeFeatureId === feature.id} onEnter={() => setActiveFeatureId(feature.id)} onLeave={() => setActiveFeatureId(null)} />
              ))}
            </div>
            <div className="relative w-full max-w-4xl h-full flex items-center justify-center z-20">
              <AnimatePresence mode="wait">
                {!activeFeatureId ? <motion.div key="hub-core" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1, filter: "blur(20px)" }} transition={{ duration: 0.5 }} className="flex items-center justify-center"><NeuralCore /></motion.div> : <motion.div key="detail-card" initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 1.1, filter: "blur(10px)" }} transition={{ duration: 0.4, ease: "easeOut" }} className="w-full"><CapabilityDetailView cap={activeFeature!} /></motion.div>}
              </AnimatePresence>
            </div>
          ...
          <div className="sticky top-0 h-screen w-full flex items-center overflow-hidden">
            ...
            <div className="container mx-auto px-6 relative z-10">
              <div className="max-w-4xl mb-20">
                ...
                <h2 className="text-6xl md:text-8xl font-black tracking-tighter leading-none uppercase">The Intelligence <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-sinai-glow-orange/30">Pipeline.</span></h2>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 items-center">
                  ...
                  <div className="absolute inset-0 rounded-[4rem] overflow-hidden border border-white/10 bg-zinc-950 shadow-2xl group">
                    <AnimatePresence mode="wait">
                      {PIPELINE_STEPS.map((step, i) => (
                        <motion.div key={step.step} initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: activeStep === i ? 0.6 : 0, scale: activeStep === i ? 1 : 1.1, zIndex: activeStep === i ? 10 : 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.8, ease: "easeOut" }} className="absolute inset-0"><Image src={step.image} alt={step.title} fill className="object-cover" /></motion.div>
                      ))}
                    </AnimatePresence>
                    <div className="absolute inset-0 bg-gradient-to-br from-black via-transparent to-black/80 z-20" />
                    <div className="absolute inset-0 flex items-center justify-center z-30">
                      <div className="relative w-full h-full">
                        <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20"><motion.path d="M 100,300 Q 400,300 700,300" stroke="url(#glow-grad-myq-client)" strokeWidth="2" fill="none" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 3, repeat: Infinity, ease: "linear" }} /><defs><linearGradient id="glow-grad-myq-client" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="transparent" /><stop offset="50%" stopColor="#F2A24B" /><stop offset="100%" stopColor="transparent" /></linearGradient></defs></svg>
                ...
                <div className="lg:col-span-5 relative space-y-12">
                  {PIPELINE_STEPS.map((item, i) => (
                    <div key={i} className={`group relative space-y-6 transition-all duration-700 ${activeStep === i ? 'opacity-100 scale-105' : 'opacity-20 scale-95 blur-[1px]'}`}>
                      <div className="flex items-center gap-6"><div className={`w-14 h-14 rounded-2xl border flex items-center justify-center transition-all duration-500 ${activeStep === i ? 'bg-sinai-glow-orange text-white border-sinai-glow-orange shadow-[0_0_30px_rgba(242,162,75,0.4)]' : 'bg-white/5 border-white/10 text-zinc-500'}`}>{item.icon}</div><div className={`h-px flex-1 transition-colors duration-500 ${activeStep === i ? 'bg-sinai-glow-orange/50' : 'bg-white/5'}`} /><span className={`font-mono text-[10px] tracking-[0.5em] font-black transition-colors duration-500 ${activeStep === i ? 'text-sinai-glow-orange' : 'text-zinc-800'}`}>STEP_{item.step}</span></div>
                      <div className="space-y-4"><h3 className={`text-3xl font-black tracking-tighter uppercase transition-colors duration-500 ${activeStep === i ? 'text-white' : 'text-zinc-600'}`}>{item.title}</h3><p className={`text-base font-light leading-relaxed transition-colors duration-500 ${activeStep === i ? 'text-zinc-400' : 'text-zinc-700'}`}>{item.desc}</p></div>
      ...
      <section className="py-64 relative overflow-hidden bg-[#06080a]">
        ...
        <div className="absolute inset-20 border border-white/[0.03] pointer-events-none hidden md:block">
          <div className="absolute top-0 left-0 w-20 h-px bg-sinai-glow-orange/30" /><div className="absolute top-0 left-0 w-px h-20 bg-sinai-glow-orange/30" /><div className="absolute bottom-0 right-0 w-20 h-px bg-sinai-glow-orange/30" /><div className="absolute bottom-0 right-0 w-px h-20 bg-sinai-glow-orange/30" />
        </div>
        <div className="container mx-auto px-6 relative z-10">
          <SectionReveal>
        ...
        <div className="absolute bottom-12 right-12 text-[8px] font-mono text-zinc-800 tracking-widest hidden lg:block uppercase">Apex_Experts_Engineering_Studio<br />System_Build_Hash: 0xPR_MYQUERY</div>
      </section>
