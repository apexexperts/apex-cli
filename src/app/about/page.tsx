...
export default function AboutPage() {
  return (
    <div className="bg-[#050505] text-white selection:bg-sinai-glow-orange selection:text-black overflow-hidden">
      ...
      {/* The Vanguard Terminal: Future Initialization - THE SURPRISE FINALE */}
      <section className="relative py-64 overflow-hidden bg-[#050505]">
        {/* Cinematic Neural Tunnel Background */}
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150vw] h-[150vh] opacity-20">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(242,162,75,0.15)_0%,transparent_70%)] animate-pulse" />
            ...
            {[...Array(3)].map((_, i) => (
              <motion.div
                ...
                className="absolute inset-0 border border-white/[0.03] rounded-full scale-[0.8] lg:scale-[1]"
                style={{ margin: `${i * 10}%` }}
              />
            ))}
          </div>
          
          {/* Scrolling Technical Logs in Background */}
          <div className="absolute inset-0 flex justify-between px-10 opacity-[0.03] font-mono text-[8px] uppercase tracking-widest leading-relaxed overflow-hidden">
            <div className="space-y-4 animate-[scroll_60s_linear_infinite]">
              {[...Array(50)].map((_, i) => (
                <div key={i}>INIT_CORE_NODE_{i} // STATUS_OK // SYNC_0x{Math.random().toString(16).slice(2, 6)}</div>
              ))}
            </div>
            <div className="space-y-4 animate-[scroll_60s_linear_infinite_reverse]">
              {[...Array(50)].map((_, i) => (
                <div key={i}>NEURAL_LINK_STABLE // BANDWIDTH_MAX // ADAPTIVE_O2_{i}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-5xl mx-auto text-center space-y-20">
            {/* The Cinematic Core Icon */}
            <motion.div
              initial={{ scale: 0.5, opacity: 0 }}
              whileInView={{ scale: 1, opacity: 1 }}
              className="relative w-32 h-32 mx-auto"
            >
              <div className="absolute inset-0 bg-sinai-glow-orange/20 blur-3xl animate-pulse" />
              <div className="relative w-full h-full rounded-2xl border border-sinai-glow-orange/30 bg-black/40 backdrop-blur-xl flex items-center justify-center group overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-sinai-glow-orange/10 to-transparent" />
                <Image 
            ...
            <div className="space-y-10">
              <SectionReveal>
                <h2 className="text-7xl lg:text-[10rem] font-black tracking-tighter uppercase leading-none italic">
                  Initialize <br/>
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-sinai-glow-orange via-white to-white/20">The Future</span>
                </h2>
              </SectionReveal>

              <SectionReveal delay={0.2}>
                <p className="text-xl lg:text-3xl text-zinc-400 font-light max-w-3xl mx-auto leading-relaxed italic">
                  The boundary between vision and reality has dissolved. <br/>
                  <span className="text-white font-bold tracking-tight">Your transformation begins at the APEX.</span>
                </p>
              </SectionReveal>
            </div>

            {/* The Master Initialization Button */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              className="pt-10"
            >
              <Link href="/contact" className="group relative inline-flex items-center gap-6 px-16 py-8 rounded-full bg-white text-black font-black text-sm tracking-[0.4em] uppercase overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_80px_rgba(255,255,255,0.3)]">
                <span className="relative z-10">Launch Mission // 01</span>
                <div className="relative z-10 w-2 h-2 rounded-full bg-sinai-glow-orange animate-pulse" />
                <div className="absolute inset-0 bg-sinai-glow-orange translate-y-full group-hover:translate-y-0 transition-transform duration-500" />
                <span className="absolute inset-0 z-0 bg-zinc-200 opacity-0 group-hover:opacity-10 transition-opacity" />
              </Link>
            </motion.div>

            {/* Bottom Meta-Data Line */}
            <div className="pt-32 flex flex-col md:flex-row justify-center items-center gap-8 text-[9px] font-mono text-zinc-700 tracking-[0.5em] uppercase">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-sinai-glow-orange/30" />
                <span>Protocol: Engagement_Active</span>
              </div>
              <div className="hidden md:block w-px h-4 bg-white/5" />
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-sinai-glow-orange/30" />
                <span>Sync_Id: {Math.random().toString(36).slice(2, 9).toUpperCase()}</span>
              </div>
              <div className="hidden md:block w-px h-4 bg-white/5" />
              <div className="flex items-center gap-2 text-sinai-glow-orange">
                <div className="w-1.5 h-1.5 rounded-full bg-sinai-glow-orange animate-ping" />
        ...
        {/* Global Keyframes for Background Logs */}
        <style jsx global>{`
          @keyframes scroll {
